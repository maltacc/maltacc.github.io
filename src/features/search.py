...
class ScoredIndexSearch(object):
    def __init__(self, prefix, host='localhost', port=6379, db=0):
    ...
    def search(self, query_string, offset=0, count=10):
        ...
        try:
            ...
            # zrevrange returns IDs as bytes, we need thme as strings so decode
            ids = [(id.decode('utf-8'), score) for id, score in ids]  
        finally:
...
class TestIndex(unittest.TestCase):
    def test_index_basic(self):
        ...
        t.connection.flushdb()  # Clear the database for testing
        ...
        self.assertEqual(
            t.search('world'),
            ([('2', 0.0), ('1', 0.0)], 2))
        self.assertEqual(t.search('this'), ([], 0))
        self.assertEqual(
